...
{
	t_ray r = raycaster_cast(p->position, vec2d_normalize(transform), bound_check_door, p->meta);
	...
	else
	{
		...
		r = raycaster_cast(p->position, vec2d_normalize(transform), bound_check_door, p->meta);
