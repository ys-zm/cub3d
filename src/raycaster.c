...
{	
	...
	if(data->side == HIT_N || data->side == HIT_S)
	{
...
void	calculate_wall_x(t_ray_data *data, t_player *player)
{
	if (data->side == HIT_N || data->side == HIT_S)
	{
		data->wall_x = player->position.y + data->perp_wall_distance * data->ray_direction.y;
	}
	else
	{
		data->wall_x = player->position.x + data->perp_wall_distance * data->ray_direction.x;	
	}
...
void	draw_column(t_meta *meta, uint32_t col, uint32_t h)
{
	...
	calculate_wall_x(&meta->player.data, &meta->player);
	wall_texture_position(&meta->player.data, texture, meta->image->height);	
	// printf("x: %f\n", meta->player.data.texture_point.x);
	while (row < h)
	{
		// ceiling
		if (row < data.start)
